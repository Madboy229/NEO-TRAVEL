...
for (const table of TABLES) {
  try {
    ...
    console.log(`OK  ${table.padEnd(10)} — ${records.length} enregistrement(s)`);
    okCount++;
    ...
    if (table === "Matrices") {
      ...
      if (records.length === expected) {
        ...
        console.log(`    Matrices contient ${records.length} lignes (${expected} attendues)`);
      }
    }
  } catch (err) {
    console.log(`KO  ${table.padEnd(10)} — ${err.message}`);
    failCount++;
  }
}
